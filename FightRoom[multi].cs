...
{
    public partial class FightRoom_multi_ : Form
    {
        ...
        {
            get { return Other_User_Name; }
            set { Other_User_Name = value; }
        }
        ...
        public FightRoom_multi_(PokemonChoice pokechice)
        {
            InitializeComponent();
            ...
            pictureBoxrestart.Parent = pictureBoxfightroom;
            

            pokechoiceform = pokechice;

            lijamong = new Lijamong();
            gyarados = new Gyarados();
            mangnanyong = new Mangnanyong();
            mews = new Mews();
            pausegame = new PauseGame_Multi(this);
            user = new User();
            fightroombgm = new SoundPlayer("Figthroom.wav");
            panelConnect.BringToFront();

            listBoxchatting.Items.Add(">>상대방 연결이 필요합니다.");
        }
        private void buttonconnect_Click(object sender, EventArgs e)
        {
            Thread th = new Thread(new ThreadStart(InitSocket));
            th.IsBackground = true;
            th.Start();
            waittimer.Enabled = true;
            exp_check_timer.Enabled = true;
            Level_Check_timer.Enabled = true;
        ...
        {
            serverIP = IPAddress.Parse(textBoxipnum.Text);
            sPort = Int32.Parse(textBoxportnum.Text);
            serverEndPoint = new IPEndPoint(serverIP, sPort);
            try
            {
                Server = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                Server.Bind(serverEndPoint);
                Server.Listen(2);
                listBoxchatting.Items.Add(">>상대방 연결대기 중 입니다.");

                Client = Server.Accept();

                if (Client.Connected)
                {
                    Client.Receive(getByte,SocketFlags.None);
                    ...
                    if (stringbyte != String.Empty)
                    {
                        ...
                        other_user_name = stringbyte;
                        listBoxchatting.Items.Add(other_user_name + "님께서 입장하였습니다.");
                        stringbyte = pokechoiceform.user_name;
                    ...
                    while (true)
                    {
                        ...
                        if (stringbyte != String.Empty)
                        {
                            ...
                            getValueLength = byteArrayDefrag(getByte);
                            stringbyte = Encoding.UTF7.GetString(getByte, 0, getValueLength + 1);
                            listBoxchatting.Items.Add(">>"+other_user_name+": " + stringbyte);
                        }
                        getByte = new byte[1024];
                        setByte = new byte[1024];
                    }
            ...
            {
                MessageBox.Show("[Error]:{0}", socketEx.Message);
            }
            catch (System.Exception commonEx)
            {
                MessageBox.Show("[Error]:{0}", commonEx.Message);
            }
            finally
            {
                Server.Close();
        ...
        private void MultiForm_Load(object sender, EventArgs e)
        {
            ...
            exp_check_timer.Enabled = false;
            Level_Check_timer.Enabled = false;
        ...
        private void Send_Message(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                sendstring = textBoxchat.Text;
                if (sendstring != String.Empty)
                {

                    setByte = Encoding.UTF7.GetBytes(sendstring);
                    Client.Send(setByte);
                    listBoxchatting.Items.Add(pokechoiceform.user_name+": "+sendstring);
                }
        ...
        private void MultiForm_Close(object sender, FormClosingEventArgs e)
        {
            Application.Exit();
        }
        ...
        private void buttonhp_potion_Click_1(object sender, EventArgs e)
        {
            if (user_hp_potion <= 0)
            {
